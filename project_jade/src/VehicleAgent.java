...
public class VehicleAgent extends GuiAgent {
	...
	protected void setup() {
		...
		if (args.length == 2) {
			...
			//agentShape.setUserData(this.getAID().getName());
			...
			if(carType==Container.ambulanceCarCost) {
				agentShape.setFill(Color.YELLOW);
				speed = 1.5;
...
//private boolean checkShapeCollision(Shape thisAgent) {
//boolean collisionDetected = false;
//
//for (Shape otherAgent : gui.agentShapeList) {
//	if (thisAgent != otherAgent) {
//		Shape intersect = Shape.intersect(otherAgent, thisAgent);
